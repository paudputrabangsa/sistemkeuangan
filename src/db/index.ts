...
export class AppDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(14).stores({
        ...
        await tx.table('sync_queue').toCollection().modify((record: Record<string, unknown>) => {
          record.status = typeof record.status === 'string' ? record.status : 'pending';
          record.group_id = typeof record.group_id === 'string' ? record.group_id : null;
