...
import { Layers, Pencil, Plus, Trash2, Settings2, ShieldCheck, FileEdit } from 'lucide-react';
...
export default function KelasPage({ fixedYearId, embedded = false }: KelasPageProps = {}) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {isTingkatFormOpen ? (
        <SectionCard title={editingTingkatId ? 'Edit Tingkat' : 'Tambah Tingkat'} description="Atur nama tingkat, tarif SPP dasar, dan kriteria usia untuk penempatan.">
      ...
      {isKelasFormOpen ? (
        <SectionCard title={editingKelasId ? 'Edit Kelas' : 'Tambah Kelas'} description="Tambahkan rombongan belajar baru. Tarif SPP akan mengikuti tingkat induknya.">
      ...
      {!isTingkatFormOpen && !isKelasFormOpen && !selectedYearLocked && tahunAjaran.length > 0 && effectiveYearId ? (
        <div className="flex justify-end mb-4">
          <button type="button" onClick={openCreateTingkat} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-brand-600/20 transition hover:bg-brand-500">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                                <span className="font-semibold">{k.activeStudentCount ?? 0} Siswa Aktif</span>
